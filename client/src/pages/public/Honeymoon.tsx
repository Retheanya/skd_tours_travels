           ...
           <div className="w-full md:w-3/5 flex flex-col justify-between py-2">
              <div>
                 <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-3">
                    <h3 className="text-3xl font-bold text-[#212529]" style={{ fontFamily: "'Cormorant Infant', serif" }}>
                       {pkg.title}
                    </h3>
                    {pkg.price && (
                       <span className="text-xl font-extrabold text-orange-500 bg-orange-50 px-4 py-2 rounded-xl border border-orange-100 shrink-0 self-start sm:self-auto" style={{ fontFamily: "'Jost', sans-serif" }}>
                          {pkg.price}
                       </span>
                    )}
                 </div>
                 ...
                 <div className="mb-6">
                    ...
                    <div className="flex gap-4">
                       ...
                       <Link to="/travels" className="w-12 h-12 rounded-full bg-orange-500 flex items-center justify-center text-white shadow-md hover:-translate-y-1 transition-transform cursor-pointer" title="Transfers">
...
const Honeymoon = () => {
  ...
  useEffect(() => {
    const fetchHoneymoonPackages = async () => {
      try {
        const honeymoonPkgs = await apiService.getHoneymoons();
        if (honeymoonPkgs.length > 0) {
          const mapped = honeymoonPkgs.map((pkg: any) => ({
            id: pkg._id,
            title: pkg.name,
            destination: pkg.subtitle || pkg.name,
            image: pkg.imageUrl,
          ...
          const uniqueFetched = mapped.filter(
            m => !honeymoonPackages.some(staticPkg => staticPkg.title.toLowerCase() === m.title.toLowerCase())
          );
          setDbPackages([...uniqueFetched, ...honeymoonPackages]);
        } else {
          setDbPackages(honeymoonPackages);
        }
      } catch (err) {
