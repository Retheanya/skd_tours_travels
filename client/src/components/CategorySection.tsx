        ...
        <div className="flex flex-wrap justify-center gap-8 md:gap-12 lg:gap-16">
          {categories.map((cat, idx) => (
            <div 
              key={idx} 
              onClick={() => {
                const route = cat.title.toLowerCase();
                if (route === "travels") navigate("/travels");
                else if (route === "pilgrimage") navigate("/packages");
                else navigate(`/${route}`);
              }}
              className="flex flex-col items-center group cursor-pointer w-[210px]"
            >
